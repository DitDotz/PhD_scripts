...
def visualize_alignment(full_template_img:np.ndarray, current_im:np.ndarray, template:np.ndarray, translation_vector:np.ndarray)->None:
    ...
    Everything is in binned so there might be accuracy errors
    '''
...
def image_to_stage_shift(px_col_vector_xy: np.ndarray, pixel_size: float, angle_deg: float) -> Point:
    """
    Convert a vector from image space to stage shift space using a specified rotation angle.
    ...
    Returns:
    Point: The transformed vector in stage shift space.
    """
    # Convert the angle from degrees to radians
    image_to_stage_shift_angle = np.radians(angle_deg)

    # Define the rotation matrix
    image_to_stage_shift_rotation_matrix = np.array([
        [np.cos(image_to_stage_shift_angle), -np.sin(image_to_stage_shift_angle)],
        [np.sin(image_to_stage_shift_angle),  np.cos(image_to_stage_shift_angle)]
    ], dtype=np.float64)
    ...
    transformed_vector = image_to_stage_shift_rotation_matrix @ vector_meters

    # Microscope coordinate system is defined from bottom left, numpy is from upper left
    reflection_factor = -1
    desired_stage_shift = StagePosition(x=transformed_vector[0][0], y=transformed_vector[1][0] * reflection_factor)

    return desired_stage_shift

def stage_to_image_shift(stage_shift_vector: StagePosition, pixel_size: float, angle_deg: float) -> np.ndarray:
    """
    Convert a vector from stage shift space back to image space using a specified rotation angle.

    Parameters:
    beam_shift_vector (Point): The input vector in stage shift space.
    pixel_size (float): The size of a pixel in meters.
    angle_deg (float): The rotation angle in degrees (default is 22.4 degrees).
    ...
    rotation_matrix_inverse = image_to_stage_shift_rotation_matrix.T

    # Create the beam shift vector as a numpy array
    stage_shift_vector_np = np.array([stage_shift_vector.x, stage_shift_vector.y])

    # Reverse the reflection on the y-coordinate
    stage_shift_vector_refl = np.array([[stage_shift_vector_np[0], stage_shift_vector_np[1] * -1]])

    # Apply the inverse rotation matrix
    vector_meters = rotation_matrix_inverse @ stage_shift_vector_refl.T # transpose to get col vectors
...
STAGE_MIN_MOTION = 1E-6  # IN METRES
PIEZO_MIN_MOTION = 10E-9

def move_stage_relative(motion: StagePosition):

    distance = (motion.x ** 2 + motion.y ** 2) ** 0.5  # In-plane distance

    if distance < PIEZO_MIN_MOTION:
        print(f"Motion {distance * 1E9:.0f}nm is too small to move")
        return
    
    if PIEZO_MIN_MOTION < distance < STAGE_MIN_MOTION:
        microscope.specimen.piezo_stage.reset_position()

        while microscope.specimen.piezo_stage.is_moving:
            continue

        print(f"Motion {distance * 1E9:.0f}nm is PIEZO")
        original_x, original_y = microscope.specimen.stage.position.x, microscope.specimen.stage.position.y
        microscope.specimen.piezo_stage.relative_move(StagePosition(motion.x, motion.y))
        final_x, final_y = microscope.specimen.stage.position.x, microscope.specimen.stage.position.y
        distance_travelled = ((final_x-original_x) ** 2 + (final_y - original_y) ** 2) ** 0.5
        print(f'ratio of distance travelled {distance_travelled/distance}')
        return
    
    print(f"Motion {distance * 1E9:.0f}nm is STAGE")

    microscope.specimen.stage.relative_move(motion)

# Initialize global variables
IMAGE_SIZE: int = 1024  # Assumes a square image i.e. IMAGE_SIZE_x = IMAGE_SIZE_y
DWELL_TIME:float = 50e-9 # s
...
image_to_stage_shift_angle: float = -90.8 # deg
#-23.47 for piezo movement

adorned_image_list : list = []
images_list : list = []
correction_angle_list : list = [image_to_stage_shift_angle]
...
try:
    ...
    while True:
        ...
        guard_counter += 1
        microscope.specimen.piezo_stage.enable()
        ...
        # Take image
        adorned_image : AdornedImage = microscope.acquisition.acquire_stem_image(
        ...
        desired_stage_shift = image_to_stage_shift(translation_vector_xy, PIXEL_SIZE, image_to_stage_shift_angle)
        
        move_stage_relative(desired_stage_shift)
        print('debug: stage shifted')
        ...
        # vector ratio seems out. (displaying greater than 1 when it didn't move enough)
        ...
        print(vector_from_center_im1_starting_coord_to_center_im_2_desired_coord, vector_from_center_im1_starting_coord_to_im_2_actual_coord)
        print(np.linalg.norm(vector_from_center_im1_starting_coord_to_center_im_2_desired_coord) ,np.linalg.norm(vector_from_center_im1_starting_coord_to_im_2_actual_coord))
        print(f'actual_desired_vector_magnitude_ratio: {actual_desired_vector_magnitude_ratio}') # ideal should be 1

        # correct vector magnitude before correcting vector angle
        ...
        image_to_stage_shift_angle +=correction_angle
    ...
    print(f'final angle: {image_to_stage_shift_angle}')
